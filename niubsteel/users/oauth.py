...
def _http_call(the_url, method, authorization, **kw):
    ...
    #TODO: fix the url with params
    http_url = '%s&%s&access_token=%s' % (the_url, params, authorization) if method == _HTTP_GET else the_url
    print("#####" + http_url)
    ...
    try:
        ...
        r = _parse_json(body)
        if hasattr(r, 'error_response'):
            raise APIError(r.get('error_response').get('code'), r.get('error_response').get('msg', ''), r.get('request', ''))
        return r
...
class TB_APIClient(object):
    ...
    def request_access_token(self, code, redirect_uri=None):
        ...
        if remind_in:
            ...
            if rtime < expires:
                expires = rtime
        return JsonDict(access_token=r.access_token, expires=expires, expires_in=expires, uid=r.get('taobao_user_id', None),nickname=r.get('taobao_user_nick',None))
...
class _Executable(object):
    ...
    def __call__(self, **kw):
        ...
        if method == _HTTP_POST and 'pic' in kw:
            method = _HTTP_UPLOAD
        return _http_call('%s?&format=json&method=%s' % (self._client.api_url, self._path), method, self._client.access_token, **kw)
